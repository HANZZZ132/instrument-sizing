...
def size_steam_valve(
    ...
    if iapws_props is not None:
        ...
        mu = 1.5e-5
        mw = 18.015
        logger.warning("IAPWS steam properties unavailable; ideal-gas fallback used (verify high-pressure steam with vendor software).")
    ...
    if not iapws_ok:
        warning += " Uyari: IAPWS devre disi; ideal gaz yaklasimi kullanildi. High-pressure steam vendor software ile dogrulanmali."
    if overflow:
        warning += " Gereken Cv secilebilir vana serisinin ustunde; en buyuk boyut secildi. Vendor dogrulamasi gereklidir."
    ...
    result["equations"] = [
        ...
        "x = DeltaP/P1",
        "x_choked = Fk* xTP_e  where xTP_e = xT or xTP (with reducers)",
        "IEC candidate sizing via fluids.control_valve.size_control_valve_g (steam -> actual volume via IAPWS/fallback density)",
        "Kv = 0.865 * Cv",
    ]
    result["method_panel"] = [
        "Steam sizing: kutlesel debi IAPWS (varsa) ile hesaplanan yogunluktan hacimsel debiye cevrildi; aksi halde ideal-gaz fallback kullanildi.",
        "Compressible sizing candidate-valve bazinda IEC uygulamasi ile hesaplandi.",
        "Pipe reducer etkisi icin FP ve xTP otomatik hesaplandi.",
        "Choked kontrolu xTP varsa xTP*Fk, yoksa xT*Fk uzerinden izleniyor; son sizing sonucu IEC hesaplayicisindan aliniyor.",
    ]
    result["intermediate_values"] = {
        ...
        "Laminar": laminar,
        "Z": z,
        "Z_source": "User Z; density from IAPWS or ideal-gas fallback",
    }
