...
T_r = 60.0	 			#reference temperature
Pi = 3.14159

def round_sigfigs(num, sig_figs):
    """
	Bulatkan ke angka tanda tertentu. 
 	Python yang dibangun dalam bentuk bulat adalah ke tempat desimal, sedangkan AGA memerlukan pembulatan ke angka signifikan
	"""
    if num != 0:
...
def set_units(units):
	"""
	Tetapkan satuan dan konstanta universal yang digunakan dalam perhitungan AGA 3
	"""
	global R
	global N_c
	global N_Ic
	...
	global T_r
	global Pi
	#mengembalikan unit dan menetapkan konstanta global
	units == 'US'
...
def thermal_expansion(alpha, d_r, T_r, T_f):
	...
	#alpha = koefisien muai panas
	#d_r = diameter lubang/pipa pada suhu terukur/referensi
	#T_r = suhu terukur/referensi
	#T_f = suhu aliran
	#returns d = diameter lubang pelat lubang/meter tabung pada kondisi mengalir
	d = d_r * (1 + alpha * (T_f - T_r))
	return round_sigfigs(d, 6)

def diameter_ratio(d, D):
	
	#d = diameter lubang pelat lubang pada kondisi aliran
	#D = diameter dalam meter pada kondisi aliran
	#returns beta = rasio diameter lubang pelat lubang terhadap diameter dalam tabung meter pada kondisi aliran
	beta = d / D
	return round_sigfigs(beta, 6)

def velocity_factor(beta):
	
	#beta = rrasio diameter lubang pelat lubang terhadap diameter dalam tabung meter pada kondisi aliran
...
def base_flow(C_dFT, d, dP, E_v, rho_b, rho_f, Y):
	...
	#C_dFT = koefisien debit pelat lubang konvergen
	#d = diameter lubang pelat lubang dihitung pada suhu aliran
	#dP = tekanan diferensial lubang
	#E_v = kecepatan faktor pendekatan
	#rho_b = massa jenis fluida pada kondisi basa
	#rho_f = massa jenis fluida pada kondisi mengalir
	#Y = faktor ekspansi
	#returns q_v = laju aliran massa
	global N_c #unit conversion factor (orifice flow)
	global Pi

	#Step 2.  Calculate volume flow rate
	q_v = (359.072 * C_dFT * E_v * Y * (d**2) * ((rho_f * dP)**0.5)) /  (rho_b * (10**-1))
